...
| 76 | Claude token 口径对齐（v1.3.1） | stream-json usage 解析只加 `input_tokens + output_tokens`，忽略 `cache_creation_input_tokens` 和 `cache_read_input_tokens`（典型 10k-200k/次）。导致 Claude 累计被低估 ~25×，UI 显示 Claude 120k / Codex 3M，看起来主力反了——其实 Claude 跑 3 个 phase 才是大头。修法：parser 加总四个字段（等同 Codex 的 `tokens used` 口径）。 | §16.7 |
| 77 | Track 长目标不再截断（v1.3.1） | 原 `.slice(0, 60)` 在 UI 直接砍成 60 字符。改为 CSS `white-space: pre-wrap` + 限高 + 滚动，hover 有 `title` 显示完整目标。 | §6 |
| 78 | 免费分发流程（v1.3.2） | 没有付费 Apple Developer 账号，无法通过 Gatekeeper。折中方案：① `codesign --sign -` ad-hoc 签名（不让 Gatekeeper 信任，但消除 "broken signature" 警告）；② `scripts/install.command` 一键脚本清 `com.apple.quarantine` + 拷到 `/Applications` + 启动；③ build 脚本自动产 `dist/CCCPlayer-<ver>-install.zip`（含 app + installer + README）。用户双击 install.command 即可（首次需右键→打开确认一次）。将来拿到付费证书只需改 `--sign -` 为 `Developer ID Application: <name>` + 加 `notarytool submit` 两行。 | §12、§14 |
| 79 | GOAL.md 作为唯一不可变基准（v1.4） | 把 "goal 至上、中间产物都可改" 这条哲学从隐含原则升级为所有 prompt 的显性约束。common.md 加 "Only GOAL.md is sacred — everything else is revisable evidence" 段，含 guardrail（必须给出具体证据才能 pivot，单纯 "想换个做法" 不允许）。每次 PRD 修改都要在 `## Changelog` 写一行 supersede 记录。milestone 废弃时不删，标记 `[~] ... superseded by M...: <reason>` 保留历史。 | §6.6、§17 |
| 80 | Goal 分解 + 自对抗式写 PRD（v1.4） | planning.md 加 Step 1 goal 分解（把 GOAL 拆成独立可判 done 的子目标，作为 PRD 的 `## Sub-goals` 节固化下来）+ Step 3 保存前自检（逐句对照 GOAL，确认每句话都有 PRD 覆盖链 sub-goal → scope → design → milestone）。实现路径合理性、技术选型风险、性能/平台约束都要在 Design 节预先想到。 | §17.1 |
